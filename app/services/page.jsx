    ...
    <MarketingShell>
        ...
        <div className="mx-auto grid max-w-5xl gap-4 sm:gap-6 md:grid-cols-3">
          {serviceCards.map((service) => (
            <article key={service.href} className="h-full rounded-2xl border border-slate-800 bg-slate-900/60 p-5 sm:p-6">
              <h2 className="text-xl xxs:text-lg font-semibold text-white">{service.title}</h2>
              <p className="mt-2 text-sm xxs:text-xs font-semibold uppercase tracking-wide text-amber-300">Main keyword: {service.keyword}</p>
              <p className="mt-3 break-words text-base xxs:text-sm leading-7 text-slate-300">{service.summary}</p>
              <Link
                href={service.href}
                className="mt-5 inline-flex min-h-11 items-center rounded-lg bg-slate-100 px-4 py-2 text-base xxs:text-sm font-semibold text-slate-900 transition-colors hover:bg-white"
              >
      ...
      <section className="px-4 pb-16 sm:px-6 sm:pb-20">
        <div className="mx-auto max-w-5xl rounded-2xl border border-slate-800 bg-slate-900/70 p-6 sm:p-8">
          <h2 className="text-2xl xxs:text-xl font-semibold text-white">Technical SEO checklist already implemented</h2>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <Link href="/contact" className="inline-flex min-h-11 items-center rounded-lg border border-slate-700 px-4 py-2 text-base xxs:text-sm font-medium text-slate-200 hover:border-slate-500 hover:text-white">
              Start a Project
            </Link>
          </div>
        </div>
